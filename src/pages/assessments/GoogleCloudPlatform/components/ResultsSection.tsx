...
const ResultsSection = ({ assessmentData }: ResultsSectionProps) => {
  ...
  const calculateOverallRecommendation = () => {
    ...
    if (overallScore >= 75) {
      return {
        recommendation: 'YES',
        title: 'Google Cloud Platform is an Excellent Fit for You!',
        color: 'green',
        icon: CheckCircle,
        description: 'You show strong alignment across psychological fit, technical readiness, and holistic assessment dimensions for cloud computing.'
      };
    } else if (overallScore >= 60) {
      return {
        recommendation: 'MAYBE',
        title: 'GCP Could Be Right with Preparation',
        color: 'orange',
        icon: AlertTriangle,
        description: 'You have potential but may need to strengthen certain areas before diving deep into Google Cloud Platform.'
      };
    } else {
      return {
        recommendation: 'NO',
  ...
  const careerPaths = [
    {
      ...
      skillMatch: Math.max(technical?.categories?.['Basic IT Knowledge'] || 0, 70),
      requirements: ['Infrastructure as Code', 'Networking', 'Security']
    },
    {
      title: 'Cloud Architect',
      description: 'Design scalable cloud solutions',
      skillMatch: Math.max(technical?.categories?.['Problem-Solving'] || 0, 75),
      requirements: ['System Design', 'Architecture Patterns', 'Cost Optimization']
    },
    {
      title: 'DevOps Engineer',
      description: 'Automate deployment and operations',
      skillMatch: Math.max(technical?.categories?.['Scripting Foundations'] || 0, 70),
      requirements: ['CI/CD', 'Containerization', 'Monitoring']
    },
    {
      title: 'Data Engineer',
      description: 'Build data pipelines and analytics',
      skillMatch: Math.max(wiscar?.dimensions?.['Real-World Alignment'] || 0, 65),
      requirements: ['Data Processing', 'SQL/NoSQL', 'ETL Pipelines']
    }
  ];
  ...
  const learningPath = [
    {
      stage: 'Foundation',
      modules: ['GCP Fundamentals', 'Cloud Console Navigation', 'Core Services Overview'],
      duration: '2-4 weeks',
      completed: false
    },
    {
      stage: 'Intermediate',
      modules: ['Compute Engine', 'Cloud Storage', 'Networking & Security'],
      duration: '6-8 weeks',
      completed: false
    },
    {
      stage: 'Advanced',
      modules: ['Kubernetes Engine', 'Cloud Functions', 'BigQuery & Analytics'],
      duration: '8-12 weeks',
      completed: false
    },
    {
      stage: 'Certification',
      modules: ['Professional Cloud Engineer Prep', 'Practice Projects', 'Portfolio Building'],
      duration: '4-6 weeks',
      completed: false
    }
  ];
  ...
  const alternatives = [
    { title: 'AWS (Amazon Web Services)', reason: 'Market leader with extensive service catalog' },
    { title: 'Azure (Microsoft)', reason: 'Strong enterprise integration and Windows focus' },
    { title: 'Data Science', reason: 'Analytics focus with cloud computing elements' },
    { title: 'Cybersecurity', reason: 'Security specialization with cloud knowledge' }
  ];
          ...
          <CardContent>
            ...
            <div className="mt-6 p-4 bg-blue-50 rounded-lg">
              ...
              <ul className="space-y-1 text-sm text-blue-800">
                <li>• Sign up for Google Cloud Platform (free tier)</li>
                <li>• Complete "GCP Fundamentals" course on Coursera</li>
                <li>• Practice with Qwiklabs hands-on labs</li>
                <li>• Join the Google Cloud Community</li>
              </ul>
            </div>
          </CardContent>
        </Card>
